...
- [ ] **素体表現: メッシュSDF コライダ** — カプセル近似は円断面で胴・足を表せず、ヌルテストで偽貫通が出る(実機 E2E 確認)。素体メッシュからセッション内で SDF を構築する衝突表現へ移す(保存しない=No Cache 両立、[docs/DESIGN.md](docs/DESIGN.md) §6)。**E2E のゲート**。実装可否はスパイクで検証
